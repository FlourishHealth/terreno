...
export const PopoverDemo = (props: Partial<PopoverProps>) => {
  ...
  return (
    <Box alignItems="center" direction="column" gap={4}>
      <Button
        iconName="file-lines"
        onClick={() => setVisible((current) => !current)}
        text={visible ? "Hide document" : "Show document"}
        variant="outline"
      />
      {visible ? (
        <Popover
        ...
        <Text color="secondaryLight">Press the button to open the document preview.</Text>
      )}
    </Box>
  );
};
