...
description: >-
  Iterate recent GitHub issues, propose which to solve, pause for confirmation
  and clarifying questions, post the Pick plan as an issue comment, then
  implement with terreno-2-pick and roast against that comment.
  Trigger with /work-github-issues or phrases like "work recent issues",
  "pick a GitHub issue", "plan which issues to solve".
...
- Unattended pickup of `status:ready-for-dev` — use `implement-ready-for-dev` (plan + draft PR) or `autobot-ready-for-dev` (Grow through Taste)
