...
import type {SyncDbDevtoolsRegistry, SyncDbDevtoolsSnapshot} from "./devtools";

const getRegistry = (): SyncDbDevtoolsRegistry | undefined => {
  return (
...
describe("SyncDB devtools registry", () => {
  it("registers debug clients with debugger state and mutation controls", async (): Promise<void> => {
    ...
    const snapshot = bridge?.snapshot() as SyncDbDevtoolsSnapshot;
