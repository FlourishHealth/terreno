---
name: pre-commit
description: Run lint, type check, and related tests on the current working tree. Fix any failures encountered. Use before commits and after CI fix attempts.
---
...
5. Find and run related tests:
   ...
   - Run only the discovered tests:
     ...
     bun test <files>
     ```
   - If changes are concentrated in one package, inspect that package's scripts and
     prefer its package-level test command.
   - If no specific tests are found, run the documented affected-package suites.
