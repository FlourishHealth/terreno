...
refilling work as PRs merge — use `terreno-scan-loop` instead. This skill runs the rounds
...
- Requires the Terreno lifecycle plugin (`terreno-1-grow` … `terreno-5-taste`). If it is
...
Aim (once per goal)
  → Sweep → Sift
      ...
      → findings → Plot → handoff
          → terreno-1-grow (slice brief → approved IP + tasks)
          → terreno-pick-roast-loop, or terreno-2-pick once
          → terreno-4-brew → terreno-5-taste
      → Track
...
4. **Run the handed-off slice.** For the slice Plot names:
   - Invoke `terreno-1-grow` with the slice brief. Grow owns the IP, the task list, and
     human approval.
   - On Grow `PASS`, invoke `terreno-pick-roast-loop` when it is installed, otherwise
     `terreno-2-pick` once — Pick owns the pick → roast → next-task inner loop.
   - On completion, invoke `terreno-4-brew`, then `terreno-5-taste`, honoring their
