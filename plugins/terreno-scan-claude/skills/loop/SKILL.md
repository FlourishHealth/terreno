...
- Requires the lifecycle plugin (`1-grow` … `5-taste`) and a host CLI
...
1. **Resolve the campaign.** Read the charter and campaign state for the goal. Verify the
   ...
   invoke `1-aim` and stop at its approval gate.
...
2. **Refill capacity.** When open campaign PRs are below the effective `wipLimit` and the
   ...
   - planned findings remain → `4-plot` for the next slice
   - backlog empty, rounds remain → `2-sweep`, then `3-sift`
   - a slice is ready to build → `1-grow` with its brief, then
     `pick-roast-loop` (or `2-pick` once), then `4-brew`
