...
[`5-taste`](../5-taste/SKILL.md). Taste performs one reactive iteration
...
- Sibling `5-taste` is available from this plugin (it ships beside this skill).
...
3. **Confirm Taste is usable.** Sibling `5-taste` ships in this plugin. If that
   skill file is missing from the installed plugin, stop and report
   `skipped — 5-taste unavailable` for every qualifying PR. Do not fall back to
...
4. **Isolate each qualifying PR in its own worktree.** Never let two Taste invocations
   ...
   `/tmp/taste-sweep/<owner>-<repo>-pr<number>`.
...
5. **Dispatch one worker per qualifying PR, in parallel.** Each worker owns one
   ...
   - **Loop:** invoke `5-taste` against this PR until `PASS` or `BLOCKED`.
...
Load `5-taste` for every qualifying PR. Discover repository skills only inside
...
- `5-taste` missing from the installed plugin
