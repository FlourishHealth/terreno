...
2. A bare comma-separated list of allowed tokens (`grow,roast` or
   `grow, pick, brew`)
3. Tokens after `/planning-loop` or the skill name
...
`1-grow` once (shape / confirm the list). Skip Grow when a current
...
If `pick` is selected, invoke `2-pick` **once**. Pick:
...
If `roast` is selected and `pick` is **not**, invoke `3-roast`
...
owned by Pick. Do not invoke `3-roast` as a second driver.
...
- `next: pick` — invoke `2-pick` (which will Roast again as part of
  its inner loop)
- `next: roast` — invoke `3-roast` (only when Pick is not driving)
- `next: brew` — invoke `4-brew` again for a submission-only retry; honor
  `wait` when provided
...
If `taste` is selected, invoke `5-taste` after Brew `PASS` (or
