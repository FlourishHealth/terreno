...
import {orgContextMiddleware, resolveOrgContext} from "./orgContext";
...
describe("org security defects (reproduction)", () => {
  let app: Application;
  ...
  describe("Defect A — disabled organizations still usable", () => {
    it("resolveOrgContext accepts a disabled organization via explicit header (platform operator)", async () => {
      ...
      assert.isDefined(context.organization);
      assert.isTrue(context.organization?.disabled);
      assert.isUndefined(context.membership);
    });

    it("GET /orgs/mine lists disabled organizations for platform operators", async () => {
      const operator = await createUser({email: "operator@example.com", roles: ["operator"]});
      ...
      assert.isTrue(mine.body.data[0].disabled);
    });

    it("operator can select a disabled org via X-Organization-Id and list tenant data", async () => {
      const operator = await createUser({email: "operator@example.com", roles: ["operator"]});
      const org = await Organization.create({
        disabled: true,
        name: "Disabled Co",
      ...
      assert.equal(res.body.data[0].title, "Secret project");
    });

    it("operator can attach a member to a disabled organization", async () => {
      const operator = await createUser({email: "operator@example.com", roles: ["operator"]});
    ...
    it("operator can reactivate a suspended member while the organization stays disabled", async () => {
      const operator = await createUser({email: "operator@example.com", roles: ["operator"]});
    ...
    it("deleted organizations are rejected by resolveOrgContext (404)", async () => {
      const operator = await createUser({email: "operator@example.com", roles: ["operator"]});
  ...
  describe("Defect B — organizationId is client-writable on update", () => {
    it("REST PATCH moves a document to another organization when the caller belongs to both", async () => {
      ...
      assert.equal(String(patched.body.data.organizationId), String(orgB._id));

      const reloaded = await OrgProjectModel.findById(project._id);
      assert.equal(String(reloaded?.organizationId), String(orgB._id));
    });
  });
});
