...
description: 'Morning job: probe for a newer Expo SDK beta, fail fast when none exists and no loop is open, otherwise create/continue/resume release-X.Y.Z, keep a loop log of tries and release notes, upgrade Expo, and land native fingerprint deps. Trigger with /track-upstream-expo, "track upstream expo", "check expo beta", or "morning expo probe".'
