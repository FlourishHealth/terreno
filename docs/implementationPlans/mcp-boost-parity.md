...
**Related work:** `docs/implementationPlans/model-router-mcp.md` adds MCP tools to consumer apps' own APIs — that is the *app's* MCP surface; this plan is about the *development-time* MCP surface. Laravel MCP (`laravel/mcp`) parity for that app surface is [`app-mcp-laravel-parity.md`](app-mcp-laravel-parity.md). The companion plan `docs-site-and-versioning.md` covers versioned documentation, which `terreno_search_docs` will consume once it exists.
