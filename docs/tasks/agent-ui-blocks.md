...
**Status:** Draft — blocked on IP approval (`docs/implementationPlans/agent-ui-blocks.md`).
...
- [ ] **Task 1.2**: Datasets, chart and table schema, semantic lint
  - Delivers: `datasets` map with typed columns and rows; `chart` (`kind`, `data`/`x`/`y` or `points`) and `table` blocks; lint pass for dataset refs, column existence/type, row arity, id uniqueness, D7 limits, chart heuristics as warnings.
  ...
  - Blocked by: 1.1
  - Docs: `docs/reference/blocks.md` (dataset + chart + table sections, warning codes).
  - Acceptance: AC2 for semantic codes (`DATASET_NOT_FOUND`, `COLUMN_NOT_FOUND`, `COLUMN_TYPE_MISMATCH`, `ROW_ARITY_MISMATCH`, `DUPLICATE_ID`, `DATASET_TOO_LARGE`, `TABLE_TOO_WIDE`) and warnings (`BAR_TOO_MANY_CATEGORIES`, `DONUT_TOO_MANY_SLICES`, `LINE_SINGLE_POINT`); AC3 perf test; doc-code parity test reads `docs/reference/blocks.md` and asserts every code in `BLOCK_ERROR_CODES` appears.

- [ ] **Task 1.3**: Actions schema, fence extraction, JSON Schema, prompt section, CLI
  - Delivers: `actions` block with `button`/`segmented` elements and closed `action.kind` set (`reply`, `open`, `select`, `callback`); `extractBlockFences(markdown)`; `blocksJsonSchema`; `blocksPromptSection({hostActions})` that embeds the grammar, limits, and one example; `terreno-blocks validate` CLI bin.
...
- [ ] **Task 2.2**: Chart and table blocks bound to datasets
  - Delivers: `chart` renders `BarChart`/`LineChart`/`AreaChart`/`DonutChart` inside a `Card` with title, legend, `emptyText`, `height` map (`sm`/`md`/`lg` → px); `table` renders `DataTable` with typed columns; per-point `color` is not exposed to the agent.
  - Files: `ui/src/blocks/blockRenderers.tsx`, `ui/src/blocks/datasetToPoints.ts`, tests, `demo/stories/BlocksView.stories.tsx` (chart presets).
  ...
  - Acceptance: AC5 for chart/table; `datasetToPoints` unit tests (date columns formatted with Luxon, numeric parse, empty dataset → `emptyText`); screenshot of bar + donut from one dataset.

- [ ] **Task 2.3**: Actions, `onAction`, segmented dataset switch
  - Delivers: `actions` renders `Button` / `SegmentedControl`; every press calls `onAction({blockId, elementId, action})`; `select` updates local dataset binding of the target block and still reports; `hostActions` allowlist marks unknown `callback` names as disabled with tooltip.
  - Files: `ui/src/blocks/BlocksView.tsx`, `ui/src/blocks/useBlockSelections.ts`, `ui/src/Common.ts`, tests, story additions.
  - Blocked by: 1.3, 2.2
  - Docs: `docs/reference/ui.md` (`onAction` payload table), `docs/explanation/agent-ui-blocks.md` (interaction flow).
  - Acceptance: AC8; `onAction` payload snapshot per kind; disabled unknown callback asserted.

- [ ] **Task 2.4**: `MarkdownView.fenceRenderers`, `GPTChat` wiring, Blocks Playground
  - Delivers: `MarkdownView` accepts `fenceRenderers`; `GPTChat` renders ` ```terreno-ui ` fences as `BlocksView`, shows a `Spinner` row while a fence is open during streaming, forwards `reply` to `onSubmit` and everything else to `onBlockAction`; demo Blocks Playground (YAML `TextArea` → live render + error list, fixture presets).
  ...
  - Blocked by: 2.3
  - Docs: `docs/reference/ui.md` (`GPTChat.onBlockAction`, `MarkdownView.fenceRenderers`), `docs/how-to/agent-ui-blocks.md` (playground section).
  - Acceptance: AC7; open-fence placeholder test (message content ending mid-fence renders `Spinner`, not raw YAML); playground recording saved under `/opt/cursor/artifacts/`.

### Phase 3: Producer — `@terreno/ai`

- [ ] **Task 3.1**: `uiBlocks` route option, prompt constant, post-stream validation SSE
  - Delivers: `addGptRoutes(router, {uiBlocks})` appends `TERRENO_UI_BLOCKS_SYSTEM_PROMPT` (built from `blocksPromptSection` + host callback names); after `text-delta` stream ends, every fence is validated and SSE `{blocks: {index, ok, errors, warnings}}` is emitted before `{done}`; `AIRequest.metadata.uiBlocks` summary logged.
  - Files: `ai/package.json` (`@terreno/blocks` dep), `ai/src/service/prompts.ts`, `ai/src/routes/gpt.ts`, `ai/src/types/index.ts`, `ai/src/routes/gpt.test.ts`.
  - Blocked by: 1.3
  - Docs: `docs/reference/ai.md` (option + SSE event row), `docs/how-to/agent-ui-blocks.md` (backend enable section).
  - Acceptance: AC9 with a mock model that streams prose + a valid fence + an invalid fence; prompt constant lives at the top of `prompts.ts` (ai-prompt-governance); no SSE `blocks` event when `uiBlocks` is off (regression test).
...
- [ ] **Task 4.1**: example-frontend AI tab renders blocks and handles actions
  - Delivers: `ai.tsx` handles SSE `blocks` event (attaches validation summary to the message for a subtle "n components" caption), passes `onBlockAction` (`open` → `router.push` for `/…` routes or `Linking.openURL` for https; `callback` → console.info stub), `reply` flows through existing `onSubmit`; e2e mock streams a fence.
  - Files: `example-frontend/app/(tabs)/ai.tsx`, `example-frontend/e2e/helpers/mockGpt.ts`, `example-frontend/e2e/ai-chat.spec.ts`.
  - Blocked by: 2.4, 3.1
  - Docs: `docs/how-to/agent-ui-blocks.md` (frontend wiring section), `docs/explanation/example-coverage.md` (capability row).
  - Acceptance: AC13; recording of prompt → bar chart → follow-up button → new user message saved under `/opt/cursor/artifacts/`.

- [ ] **Task 4.2**: example-backend enable, changelog, docs sweep
  - Delivers: `example-backend` passes `uiBlocks: {hostActions: ["exportDataset"]}`; changelog entry; all reference/how-to/explanation pages cross-linked; rules regenerated.
  - Files: `example-backend/src/server.ts`, `changelog/unreleased/agent-ui-blocks.md`, `docs/how-to/README.md`, `docs/reference/README.md`, `docs/explanation/README.md`, `.rulesync/rules/ui/00-ui.md` (BlocksView bullet) → `bun run rules`.
