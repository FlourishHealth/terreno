...
- [ ] **Task 1.1**: Write `docs/explanation/positioning.md`
  - Description: New explainer containing four clearly-labeled copy blocks in fenced code blocks (so they can be copied verbatim without markdown rendering surprises): `tagline` (one line), `elevator` (2–3 sentences), `pillars` (three bullets, in the order batteries-included / universal-by-default / AI-native), and `pitch` (~150 words). Then a "Why the Django/Rails analogy" section, then the honest comparison table from the IP. Then a "Language rules" section restating the four rules from the program doc (lead with the analogy, say "universal app", never claim unshipped features, keep superlatives off reference pages).
  - Files: `docs/explanation/positioning.md` (new)
  - Depends on: none
  - Acceptance: four fenced copy blocks present and labeled; comparison table has at least eight rows including at least three honest gaps with IP links; language rules section present.

- [ ] **Task 1.2**: Verify every claim in the comparison table
  - Description: For each row of the comparison table, confirm the Terreno side exists by finding it in source: `modelRouter` in `api/src/api.ts`, admin packages, `Permissions` in `api/src/permissions.ts`, Better Auth in `api/src/betterAuth*.ts`, MCP tools in `mcp-server/src/tools.ts`. For each "not shipped" row (background jobs, SSR, RBAC), confirm it is genuinely absent and link the IP or roadmap item tracking it. Correct any row that turns out to be wrong.
  - Files: `docs/explanation/positioning.md`
  - Depends on: Task 1.1
  - Acceptance: every "shipped" row cites a source path; every "not shipped" row links an IP file that exists in `docs/implementationPlans/`.
