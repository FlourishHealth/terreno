...
- [ ] **Task 1.2**: Add public `GET /version-check` endpoint
  - Description: Add a built-in public route to TerrenoApp/setupServer that handles `GET /version-check?version=N&platform=web|mobile`. Fetches the singleton VersionConfig, compares the client's build number against the platform-specific thresholds, and returns `{status, message?, updateUrl?}`. Returns `{status: "ok"}` if no config document exists.
  - Files: `api/src/terrenoApp.ts` or `api/src/setupServer.ts` (modify), `api/src/routes/versionCheck.ts` (create)
  - Depends on: Task 1.1
