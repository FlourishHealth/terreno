...
and Taste each implement one bounded transition. Pick and Roast continue an inner loop
until the approved task list is done.
...
| `terreno-3-roast` | Pick result + current diff | independent requirement/evidence verdict for the current task | Pick if tasks remain, else Brew |
