---
name: check-watcher
description: Legacy standalone CI-only watcher for explicit use outside the bounded lifecycle. Not used by Taste; lifecycle waiting/reinvocation belongs to the outer loop.
...
5. If failure IS related to PR changes:
   ...
   - Commit and push (`git commit -s` for DCO; no AI attribution, no conventional prefixes)
   - Return to step 2
