...
description: |
  ...
  api (Express/Mongoose), ui (React Native components), rtk (Redux Toolkit Query), demo,
  example-frontend (Expo), example-backend (Express), and mcp-server.
...
steps:
  ...
  - name: Start MongoDB (needed for api and example-backend tests)
...
| Package | Type | Test Command | Notes |
|---------|------|-------------|-------|
| `api/` | Backend (Express/Mongoose) | `bun test --preload ./src/tests/bunSetup.ts` | Requires MongoDB |
| `ui/` | React Native components | `TZ=America/New_York bun test` | Uses @testing-library/react-native |
| `rtk/` | Redux Toolkit Query utils | No tests yet | Good coverage opportunity |
| `example-backend/` | Example Express app | `bun test` | Requires MongoDB |
| `example-frontend/` | Example Expo app | `bun test` | Minimal tests |
| `mcp-server/` | MCP server | `bun test` | Has basic tests |
| `demo/` | Demo app | No tests | Low priority |

**Key details:**
- **Package manager**: Bun (use `bun install`, not `npm`/`yarn`)
- **Test runner**: `bun test` (Jest-compatible API with `expect`)
- **Coverage**: `bun test --coverage` produces coverage output
- **Linter**: Biome (`bun run lint` / `bun run lint:fix`)
- **MongoDB**: Required for `api/` and `example-backend/` tests (start with `supercharge/mongodb-github-action`)
- **Monorepo root scripts**: `bun run test` runs all packages, `bun run api:test` / `bun run ui:test` for individual packages
- **Test file convention**: `*.test.ts` / `*.test.tsx` co-located with source files
- **UI test helper**: `renderWithTheme()` from `ui/src/test-utils.tsx` wraps components in ThemeProvider
...
2. Create a discussion with title "${{ github.workflow }} - Research and Plan" that includes:
  ...
  - Details of the commands needed to run to build the project, run tests, and generate coverage reports (use `bun test --coverage` for each workspace)
  - Details of how tests are organized in the repo (co-located `*.test.ts`/`*.test.tsx` files), and how new tests should be organized
  - Per-package coverage breakdown (api, ui, example-backend, mcp-server have tests; rtk, example-frontend, demo have minimal or no tests)
...
2. Have a careful think about the CI commands needed to build the repository, run tests, produce a combined coverage report and upload it as an artifact.
   ...
   **Important context for this monorepo:**
   ...
   - MongoDB must be running for api and example-backend tests (already started in workflow steps above)
   - Use `bun test --coverage` in each workspace to produce coverage output
   - Run tests per-workspace: `cd api && bun test --coverage --preload ./src/tests/bunSetup.ts`, `cd ui && TZ=America/New_York bun test --coverage`, `cd example-backend && bun test --coverage`, `cd mcp-server && bun test --coverage`
   - The API package must be compiled before example-backend tests: `cd api && bun run compile`
   - Linting uses Biome: `bun run lint` from root

   Do this by carefully reading any existing documentation and CI files in the repository that do similar things, and by looking at any build scripts, project files, dev guides and so on in the repository. Organize the steps in order as a series of YAML steps suitable for inclusion in a GitHub Action.
...
1. **Goal selection**. Build an understanding of what to work on and select an area of the test coverage plan to pursue
   ...
   **Priority order for coverage improvements:**
   1. `api/` - Core backend framework, most impactful to test thoroughly
   2. `ui/` - Already has many tests, focus on uncovered branches and edge cases
   3. `rtk/` - Currently has no tests, high opportunity
   4. `example-backend/` - Integration tests for the framework
   5. `mcp-server/` - Has basic tests, can be expanded
   6. `example-frontend/` - Minimal tests, lower priority
...
2. **Work towards your selected goal**. For the test coverage improvement goal you selected, do the following:
   ...
   **Testing conventions for this repo:**
   - Use `bun test` with `expect` assertions (Jest-compatible API)
   - Test files are co-located: `src/MyFile.ts` -> `src/MyFile.test.ts`
   - API tests: use `supertest` for HTTP testing, `getBaseServer()` and `authAsUser()` helpers from `api/src/tests.ts`, `setupDb()` for test database
   - UI tests: wrap components in `renderWithTheme()` from `ui/src/test-utils.tsx`, use `@testing-library/react-native`
   - Backend tests need MongoDB running
...
3. **Finalizing changes**
   ...
   a. Run `bun run lint:fix` from the repo root to apply Biome formatting.
   
   b. Run `bun run lint` to ensure no new linting errors remain.
...
4. **Results and learnings**
   ...
   a. If you succeeded in writing useful code changes that improve test coverage, create a **draft** pull request with your changes.
      ...
      **Critical:** Exclude coverage reports and tool-generated files from PR. Double-check added files and remove any that don't belong. Never include changes to `example-frontend/store/openApiSdk.ts` (auto-generated).
      ...
      **Reproducibility section:**
      Provide clear instructions to reproduce coverage testing, including setup commands (install dependencies, build code, run tests, generate coverage reports), measurement procedures, and expected results format. Example:
      ```
      bun install --frozen-lockfile
      cd api && bun run compile
      cd api && bun test --coverage --preload ./src/tests/bunSetup.ts
      cd ui && TZ=America/New_York bun test --coverage
      ```
